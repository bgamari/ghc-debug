...
parseInternal :: HeapGraphIndex -> DebugClosure ConstrDesc s (Maybe HeapGraphIndex) -> PrintState [VisObject]
parseInternal _ (ConstrClosure _ [] [dataArg] (ConstrDesc{modl, name})) =
...
parseInternal _ (ConstrClosure _ bPtrs dArgs (ConstrDesc _ _ name))
...
showClosureFields :: DebugClosure ConstrDesc s t -> [String]
showClosureFields (ConstrClosure _ _ [dataArg] (ConstrDesc{modl, name})) =
 case (modl, name) of
    k | k `elem` [ ("GHC.Word", "W#")
                 , ("GHC.Word", "W8#")
                 , ("GHC.Word", "W16#")
