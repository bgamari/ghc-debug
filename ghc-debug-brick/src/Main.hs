...
completeClosureDetails :: Debuggee -> (Text, DebugClosure SrtCont PayloadCont ConstrDescCont StackCont ClosurePtr)
                                            -> IO ClosureDetails
  ...
  asyncAction "Counting ARR_WORDS" os (arrWordsAnalysis Nothing dbg) $ \res -> do
    let sorted_res = take 100 $ Prelude.reverse [(k, S.toList v ) | (k, v) <- (List.sortBy (comparing (S.size . snd)) (M.toList res))]
        ...
        g_children d (CountLine b _) = do
          ...
          cs' <- closureReferences' d (zipWith (\n c -> (show n, Left c)) [0..] (S.toList cs))
          children' <- traverse (traverse (fillListItem d)) cs'
